...
export default (async ({ req, next }) => {
    ...
    |--------------------------------------------------------------------------------------------
    */
    const tags = async (entries: string[]): Promise<string> => {
        if (!PRODUCTION) {
            if (!vite) throw new Error('ViteDevServer belum terinisialisasi.');
            return resolveViteDevTags(req.originalUrl, entries, vite);
        }
    ...
    |--------------------------------------------------------------------------------------------
    */
    const ssrRender = async (page: object): Promise<{ body: string }> => {
        if (!PRODUCTION) {
            if (!vite) throw new Error('ViteDevServer belum terinisialisasi.');
