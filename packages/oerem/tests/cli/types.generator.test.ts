...
import { hasMany, belongsTo, belongsToMany } from '../../src/schema/relations.js'
...
const roleModel: DiscoveredModel = {
...
const userRoleModel: DiscoveredModel = {
    filePath: '/project/models/user-role.model.ts',
    fileName: 'user-role.model.ts',
    modelName: 'user-role.model',
    def: {
        identifier: 'UserRole',
        table: 'user_role',
        schema: {
            id: field.id().build(),
            user_id: field.foreignId().constrained('users').cascadeOn('delete').build(),
            role_id: field.foreignId().constrained('roles').cascadeOn('delete').build(),
        },
    },
}
...
describe('generateTypeFile', () => {
    ...
    it('generates QUser query builder type', () => {
        const content = generateTypeFile(userModel, [userModel, postModel], '/output')
        expect(content).toContain('export type QUser = OeremQueryBuilder<TUser, RUser>')
    })

    it('imports OeremQueryBuilder from oerem', () => {
        const content = generateTypeFile(userModel, [userModel, postModel], '/output')
        expect(content).toContain("import type { OeremQueryBuilder } from '@lalase/oerem'")
    })
    ...
    it('generates belongsToMany with pivot as intersection type', () => {
        // Create a user model with belongsToMany relation that has a pivot
        const userWithRoles: DiscoveredModel = {
            filePath: '/project/models/user.model.ts',
            fileName: 'user.model.ts',
            modelName: 'user.model',
            def: {
                ...
                relations: {
                    roles: belongsToMany(() => roleModel.def, () => userRoleModel.def, 'user_id', 'role_id'),
                },
            },
        }

        const content = generateTypeFile(userWithRoles, [userWithRoles, roleModel, userRoleModel], '/output')

        // Should contain the intersection type with pivot
        expect(content).toContain('roles?: (TRole & { pivot: TUserRole })[]')
        // Should import both Role and UserRole types
        expect(content).toContain("import type { TRole } from './role.types.js'")
        expect(content).toContain("import type { TUserRole } from './userrole.types.js'")
    })
})
...
describe('generateTypes', () => {
    ...
    it('generates barrel index.ts', () => {
        ...
        expect(index).toContain("export type { TUser, RUser, QUser } from './user.types.js'")
        expect(index).toContain("export type { TPost, RPost, QPost } from './post.types.js'")
    })
})
