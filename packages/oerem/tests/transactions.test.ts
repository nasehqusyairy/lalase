...
describe('Transactions', () => {
    ...
    it('should commit all changes when a transaction completes successfully', async () => {
        ...
        const user = await ctx.db().table('users').where('username', 'alice').first();
    ...
    it('should work with Oerem Model factories inside a transaction', async () => {
        ...
        const charlie = await ctx.db().table('users').where('username', 'charlie').first();
    ...
    it('should rollback all changes if any operation within the transaction throws', async () => {
        try {
            await ctx.transaction(async () => {
                ...
                await ctx.db().table('users').insert({ username: 'bob', balance: 500 });
        ...
        const dave = await ctx.db().table('users').where('username', 'dave').first();
        const bob = await ctx.db().table('users').where('username', 'bob').first();
